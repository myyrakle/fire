...
Fire has two core concepts: providers and injectors.

### Defining Providers

The primary mechanism in Fire is the **provider**: a function that can produce a
...
called `SuperSet`, use the `fire.NewSet` function:
...
var SuperSet = fire.NewSet(ProvideFoo, ProvideBar, ProvideBaz)
```
...
var MegaSet = fire.NewSet(SuperSet, pkg.OtherSet)
```

### Injectors

An application fires up these providers with an **injector**: a function that
calls providers in dependency order. With Fire, you write the injector's
signature, then Fire generates the function's body.

An injector is declared by writing a function declaration whose body is a call
to `fire.Build`. The return values don't matter as long as they are of the
...
func initializeBaz(ctx context.Context) (foobarbaz.Baz, error) {
    fire.Build(foobarbaz.MegaSet)
    return foobarbaz.Baz{}, nil
}
```

Like providers, injectors can be parameterized on inputs (which then get sent to
providers) and can return errors. Arguments to `fire.Build` are the same as
`fire.NewSet`: they form a provider set. This is the provider set that gets used
during code generation for that injector.

Any non-injector declarations found in a file with injectors will be copied into
the generated file.

You can generate the injector by invoking Fire in the package directory:

```shell
fire
```

Fire will produce an implementation of the injector in a file called
`fire_gen.go` that looks something like this:
...
themselves. Further, there is little dependency on Fire at runtime: all of the
written code is just normal Go code, and can be used without Fire.

Once `fire_gen.go` is created, you can regenerate it by running [`go generate`].
...
an interface. Fire matches inputs to outputs via [type identity][], so the
...
var Set = fire.NewSet(
    provideMyFooer,
    fire.Bind(new(Fooer), new(*MyFooer)),
    provideBar)
```

The first argument to `fire.Bind` is a pointer to a value of the desired
...
Structs can be constructed using provided types. Use the `fire.Struct` function
...
For the resulting struct type `S`, `fire.Struct` provides both `S` and `*S`. For
...
var Set = fire.NewSet(
    ProvideFoo,
    ProvideBar,
    fire.Struct(new(FooBar), "MyFoo", "MyBar"))
```
...
The first argument to `fire.Struct` is a pointer to the desired struct type and
...
fields. So `fire.Struct(new(FooBar), "*")` produces the same result as above.
...
injector, especially when passing `*` to `fire.Struct`. You can tag a field with
`` `fire:"-"` `` to have Fire ignore such fields. For example:
...
When you provide the `Foo` type using `fire.Struct(new(Foo), "*")`, Fire will
automatically omit the `mu` field. Additionally, it is an error to explicitly
specify a prevented field as in `fire.Struct(new(Foo), "mu")`.
...
initialization. Fire will emit an error if the expression calls any functions or
...
func injectReader() io.Reader {
    fire.Build(fire.InterfaceValue(new(io.Reader), os.Stdin))
    return nil
}
```
...
You can instead use `fire.FieldsOf` to use those fields directly without writing
`getS`:
...
You can add as many field names to a `fire.FieldsOf` function as you like.
