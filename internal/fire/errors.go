...
// A fireErr is an error with an optional position.
type fireErr struct {
	error    error
...
// stack, so calling notePosition on an existing *fireErr will not modify the
...
func notePosition(p token.Position, e error) error {
	...
	case *fireErr:
		return e
	default:
		return &fireErr{error: e, position: p}
	}
}
