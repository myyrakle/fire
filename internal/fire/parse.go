...
func (p *providerSetSrc) description(fset *token.FileSet, typ types.Type) string {
	...
	case p.Binding != nil:
		return fmt.Sprintf("fire.Bind (%s)", fset.Position(p.Binding.Pos))
	case p.Value != nil:
		return fmt.Sprintf("fire.Value (%s)", fset.Position(p.Value.Pos))
	case p.Import != nil:
	...
	case p.Field != nil:
		return fmt.Sprintf("fire.FieldsOf (%s)", fset.Position(p.Field.Pos))
	}
...
type ProviderSet struct {
	// Pos is the position of the call to fire.NewSet or fire.Build that
	// created the set.
	Pos token.Pos
	...
	Fields    []*Field
	Imports   []*ProviderSet
	// InjectorArgs is only filled in for fire.Build.
...
type Value struct {
	...
	// expr is the expression passed to fire.Value.
	expr ast.Expr
...
func load(ctx context.Context, wd string, env []string, tags string, patterns []string) ([]*packages.Package, []error) {
	cfg := &packages.Config{
		...
		Dir:        wd,
		Env:        env,
		BuildFlags: []string{"-tags=fireinject"},
...
// objectCache is a lazily evaluated mapping of objects to Fire structures.
...
// get converts a Go object into a Fire structure. It may return a *Provider, an
...
// processExpr converts an expression into a Fire structure. It may return a
...
func (oc *objectCache) processNewSet(info *types.Info, pkgPath string, call *ast.CallExpr, args *InjectorArgs, varName string) (*ProviderSet, []error) {
	// Assumes that call.Fun is fire.NewSet or fire.Build.
...
func processStructLiteralProvider(fset *token.FileSet, typeName *types.TypeName) (*Provider, []error) {
	...
	fmt.Fprintf(os.Stderr,
		"Warning: %v, see https://godoc.org/github.com/myyrakle/fire#Struct for more information.\n",
		notePosition(fset.Position(pos),
			fmt.Errorf("using struct literal to inject %s is deprecated and will be removed in the next release; use fire.Struct instead",
				typeName.Type())))
	provider := &Provider{
		Pkg:      typeName.Pkg(),
		Name:     typeName.Name(),
...
func processStructProvider(fset *token.FileSet, info *types.Info, call *ast.CallExpr) (*Provider, error) {
	// Assumes that call.Fun is fire.Struct.
...
// Since this is the only tag used by fire, we can do string comparison
// without using reflect.
func isPrevented(tag string) bool {
	return reflect.StructTag(tag).Get("fire") == "-"
}

// processBind creates an interface binding from a fire.Bind call.
func processBind(fset *token.FileSet, info *types.Info, call *ast.CallExpr) (*IfaceBinding, error) {
	// Assumes that call.Fun is fire.Bind.
...
// processValue creates a value from a fire.Value call.
func processValue(fset *token.FileSet, info *types.Info, call *ast.CallExpr) (*Value, error) {
	// Assumes that call.Fun is fire.Value.
	...
	// Result type can't be an interface type; use fire.InterfaceValue for that.
...
// processInterfaceValue creates a value from a fire.InterfaceValue call.
func processInterfaceValue(fset *token.FileSet, info *types.Info, call *ast.CallExpr) (*Value, error) {
	// Assumes that call.Fun is fire.InterfaceValue.
...
// processFieldsOf creates a slice of fields from a fire.FieldsOf call.
func processFieldsOf(fset *token.FileSet, info *types.Info, call *ast.CallExpr) ([]*Field, error) {
	// Assumes that call.Fun is fire.FieldsOf.
	...
	for i := 1; i < len(call.Args); i++ {
		...
		if isPtrToStruct {
			...
			// fire.Fields also provides a pointer to the field.
...
func checkField(f ast.Expr, st *types.Struct) (*types.Var, error) {
	...
	for i := 0; i < st.NumFields(); i++ {
		if strings.EqualFold(strconv.Quote(st.Field(i).Name()), b.Value) {
			if isPrevented(st.Tag(i)) {
				return nil, fmt.Errorf("%s is prevented from injecting by fire", b.Value)
			}
...
// findInjectorBuild returns the fire.Build call if fn is an injector template.
...
func findInjectorBuild(info *types.Info, fn *ast.FuncDecl) (*ast.CallExpr, error) {
	...
	numStatements := 0
	invalid := false
	var fireBuildCall *ast.CallExpr
	for _, stmt := range fn.Body.List {
		...
		case *ast.ExprStmt:
			...
			if buildObj == nil || buildObj.Pkg() == nil || !isFireImport(buildObj.Pkg().Path()) || buildObj.Name() != "Build" {
				continue
			}
			fireBuildCall = call
		case *ast.EmptyStmt:
	...
	if fireBuildCall == nil {
		return nil, nil
	}
	if invalid {
		return nil, errors.New("a call to fire.Build indicates that this function is an injector, but injectors must consist of only the fire.Build call and an optional return")
	}
	return fireBuildCall, nil
}

func isFireImport(path string) bool {
	// TODO(light): This is depending on details of the current loader.
	const vendorPart = "vendor/"
	if i := strings.LastIndex(path, vendorPart); i != -1 && (i == 0 || path[i-1] == '/') {
		path = path[i+len(vendorPart):]
	}
	return path == "github.com/myyrakle/fire"
}
...
func isProviderSetType(t types.Type) bool {
	...
	obj := n.Obj()
	return obj.Pkg() != nil && isFireImport(obj.Pkg().Path()) && obj.Name() == "ProviderSet"
}

// ProvidedType represents a type provided from a source. The source
// can be a *Provider (a provider function), a *Value (fire.Value), or an
...
// bindShouldUsePointer loads the fire package the user is importing from their
// injector. The call is a fire marker function call.
func bindShouldUsePointer(info *types.Info, call *ast.CallExpr) bool {
	// These type assertions should not fail, otherwise panic.
	fun := call.Fun.(*ast.SelectorExpr)                 // fire.Bind
	pkgName := fun.X.(*ast.Ident)                       // fire
	fireName := info.ObjectOf(pkgName).(*types.PkgName) // fire package
	return fireName.Imported().Scope().Lookup("bindToUsePointer") != nil
}
