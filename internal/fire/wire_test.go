...
func TestFire(t *testing.T) {
	...
	fireGo, err := ioutil.ReadFile(filepath.Join("..", "..", "fire.go"))
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, ent := range testdataEnts {
		...
		test, err := loadTestCase(filepath.Join(testRoot, name), fireGo)
		if err != nil {
			t.Error(err)
			continue
		}
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			gopath, err := ioutil.TempDir("", "fire_test")
			if err != nil {
				t.Fatal(err)
			}
			...
			if len(errs) > 0 {
				...
				if !test.wantFireError {
					t.Fatal("Did not expect errors. To -record an error, create want/fire_errs.txt.")
				}
				if *record {
					fireErrsFile := filepath.Join(testRoot, test.name, "want", "fire_errs.txt")
					if err := ioutil.WriteFile(fireErrsFile, []byte(strings.Join(gotErrStrings, "\n\n")), 0666); err != nil {
						t.Fatalf("failed to write fire_errs.txt file: %v", err)
					}
				} else {
					if diff := cmp.Diff(gotErrStrings, test.wantFireErrorStrings); diff != "" {
						t.Errorf("Errors didn't match expected errors from fire_errors.txt:\n%s", diff)
					}
				}
				return
			}
			if test.wantFireError {
				t.Fatal("fire succeeded; want error")
			}
			outPathSane := true
			...
			if *record {
				// Record ==> Build the generated Fire code,
				// check that the program's output matches the
				// expected output, save fire output on
				// success.
				if !outPathSane {
					return
				}
				if err := gen.Commit(); err != nil {
					t.Fatalf("failed to write fire_gen.go to test GOPATH: %v", err)
				}
				if err := goBuildCheck(goToolPath, gopath, test); err != nil {
					t.Fatalf("go build check failed: %v", err)
				}
				testdataFireGenPath := filepath.Join(testRoot, test.name, "want", "fire_gen.go")
				if err := ioutil.WriteFile(testdataFireGenPath, gen.Content, 0666); err != nil {
					t.Fatalf("failed to record fire_gen.go to testdata: %v", err)
				}
			} else {
				...
				// Generate function.
				if !bytes.Equal(gen.Content, test.wantFireOutput) {
					gotS, wantS := string(gen.Content), string(test.wantFireOutput)
					diff := cmp.Diff(strings.Split(gotS, "\n"), strings.Split(wantS, "\n"))
					t.Fatalf("fire output differs from golden file. If this change is expected, run with -record to update the fire_gen.go file.\n*** got:\n%s\n\n*** want:\n%s\n\n*** diff:\n%s", gotS, wantS, diff)
				}
			}
		})
	}
...
//
//			fire_errs.txt
//					Expected errors from the Fire Generate function,
...
//
//			fire_gen.go
//					verified output of fire from a test run with
//					-record, missing if fire_errs.txt is present
//
//			program_out.txt
//					expected output from the final compiled program,
//					missing if fire_errs.txt is present
func loadTestCase(root string, fireGoSrc []byte) (*testCase, error) {
	...
	var wantFireOutput []byte
	fireErrb, err := ioutil.ReadFile(filepath.Join(root, "want", "fire_errs.txt"))
	wantFireError := err == nil
	var wantFireErrorStrings []string
	if wantFireError {
		wantFireErrorStrings = strings.Split(string(fireErrb), "\n\n")
	} else {
		if !*record {
			wantFireOutput, err = ioutil.ReadFile(filepath.Join(root, "want", "fire_gen.go"))
			if err != nil {
				return nil, fmt.Errorf("load test case %s: %v, if this is a new testcase, run with -record to generate the fire_gen.go file", name, err)
			}
		}
	...
	goFiles := map[string][]byte{
		"github.com/myyrakle/fire/fire.go": fireGoSrc,
	}
	...
	return &testCase{
		...
		header:               header,
		goFiles:              goFiles,
		wantFireOutput:       wantFireOutput,
		wantProgramOutput:    wantProgramOutput,
		wantFireError:        wantFireError,
		wantFireErrorStrings: wantFireErrorStrings,
	}, nil
}
...
func (test *testCase) materialize(gopath string) error {
	...
	// Add go.mod files to example.com and github.com/myyrakle/fire.
	const importPath = "example.com"
	const depPath = "github.com/myyrakle/fire"
