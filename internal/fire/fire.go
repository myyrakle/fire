...
// Package fire provides compile-time dependency injection logic as a
// Go library.
package fire
...
func Generate(ctx context.Context, wd string, env []string, patterns []string, opts *GenerateOptions) ([]GenerateResult, []error) {
	...
	for i, pkg := range pkgs {
		...
		generated[i].OutputPath = filepath.Join(outDir, opts.PrefixOutputFile+"fire_gen.go")
		g := newGen(pkg)
...
func (g *gen) frame(tags string) []byte {
	...
	buf.WriteString("// Code generated by Fire. DO NOT EDIT.\n\n")
	buf.WriteString("//go:generate go run -mod=mod github.com/myyrakle/fire/cmd/fire" + tags + "\n")
	buf.WriteString("//+build !fireinject\n\n")
	...
	for i := range calls {
		...
		if c.kind == valueExpr {
			...
			if g.values[c.valueExpr] == "" {
				...
				name := typeVariableName(t, "", func(name string) string { return "_fire" + export(name) + "Value" }, g.nameInFileScope)
