...
// This test demonstrates how to use mocks with fire.
...
var appSet = fire.NewSet(
	fire.Struct(new(app), "*"),
	fire.Struct(new(greeter), "*"),
	fire.InterfaceValue(new(timer), realTime{}),
)
...
var appSetWithoutMocks = fire.NewSet(
	fire.Struct(new(app), "*"),
	fire.Struct(new(greeter), "*"),
)
...
var mockAppSet = fire.NewSet(
	fire.Struct(new(app), "*"),
	fire.Struct(new(greeter), "*"),
	fire.Struct(new(appWithMocks), "*"),
	// For each mocked dependency, add a provider and use fire.Bind to bind
	// the concrete type to the relevant interface.
	newMockTimer,
	fire.Bind(new(timer), new(*mockTimer)),
)
