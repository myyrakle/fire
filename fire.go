...
// Package fire contains directives for Fire code generation.
// For an overview of working with Fire, see the user guide at
// https://github.com/myyrakle/fire/blob/master/docs/guide.md
//
// The directives in this package are used as input to the Fire code generation
// tool. The entry point of Fire's analysis are injector functions: function
// templates denoted by only containing a call to Build. The arguments to Build
// describes a set of providers and the Fire code generation tool builds a
...
//
// For compatibility with older versions of Fire, passing a struct value of type
...
// a future version of Fire: new providers sets should use fire.Struct.
func NewSet(...interface{}) ProviderSet {
	return ProviderSet{}
}

// Build is placed in the body of an injector function template to declare the
// providers to use. The Fire code generation tool will fill in an
// implementation of the function. The arguments to Build are interpreted the
// same as NewSet: they determine the provider set presented to Fire's
...
//		fire.Build(otherpkg.FooSet, myProviderFunc)
//		return nil, nil
//	}
//
//	func injector(ctx context.Context) (*sql.DB, error) {
//		panic(fire.Build(otherpkg.FooSet, myProviderFunc))
//	}
func Build(...interface{}) string {
	return "implementation not generated, run fire"
}
...
//	func (MyFoo) Foo() {}
//
//	var MySet = fire.NewSet(
//		fire.Struct(new(MyFoo))
//		fire.Bind(new(Fooer), new(MyFoo)))
func Bind(iface, to interface{}) Binding {
	return Binding{}
}

// bindToUsePointer is detected by the fire tool to indicate that Bind's second argument should take a pointer.
// See https://github.com/myyrakle/fire/issues/120 for details.
...
// Example:
//
//	var MySet = fire.NewSet(fire.Value([]string(nil)))
...
// Example:
//
//	var MySet = fire.NewSet(fire.InterfaceValue(new(io.Reader), os.Stdin))
...
// Foo, Fire will use field-filling to provide both Foo and *Foo. The remaining
...
//	  MyBar *Bar
//	}
//	var Set = fire.NewSet(fire.Struct(new(S), "MyFoo")) -> inject only S.MyFoo
//	var Set = fire.NewSet(fire.Struct(new(S), "*")) -> inject all fields
...
//
//	func NewStruct() S { /* ... */ }
//	var Set = fire.NewSet(fire.FieldsOf(new(S), "MyFoo", "MyBar"))
//
//	or
//
//	func NewStruct() *S { /* ... */ }
